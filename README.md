...
- **Язык разработки**: Библиотека написана на языке C стандарта C11 с использованием компилятора **gcc**.
  
- **Стиль кода**: Код оформлен в соответствии с рекомендациями **Google Style**.

- **Тип библиотеки**: Оформлена как статическая библиотека с названием *s21_matrix.a* и заголовочным файлом *s21_matrix.h*.

- **Программирование**: Разработка выполнена в соответствии с принципами структурного программирования.

- **Префиксы функций**: Каждая функция имеет префикс **s21_**.

- **Качество кода**: Полное покрытие unit-тестами функций библиотеки с использованием библиотеки **Check**.

- **Покрытие тестами**: Unit-тесты покрывают не менее **80%** каждой функции.

- **Сборка**: Подготовлен **Makefile** для сборки библиотеки и тестов, включающий цели: 
  - `all`
  - `clean`
  - `test`
  - `s21_matrix.a`
...
- **Структура матрицы**: Матрица реализована в виде структуры, описанной выше.

- **Точность**: Проверяемая точность дробной части — максимум **6 знаков** после запятой.
